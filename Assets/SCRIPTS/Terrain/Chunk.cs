...
public class Chunk
{
                    ...
                    for (int z = 0; z < BlockPerChunk; z++)
                    {
                        ...
                        float perlinMask2D =
                            Mathf.PerlinNoise(
                                ...
                                * offset
                            + Mathf.PerlinNoise(
                                (ox + position.x + World.instance.Seed + 54) / 64f,
                                (oz + position.z + World.instance.Seed + 54) / 64f
                            ) * 12f
                            + (
                                Mathf.PerlinNoise(
                                    (ox + position.x + World.instance.Seed + 586) / 512f,
                                    (oz + position.z + World.instance.Seed + 586) / 512f
                                ) * 10f
                            )
                                * (
                                    Mathf.PerlinNoise(
                                        (ox + position.x + World.instance.Seed + 206) / 64f,
                                        (oz + position.z + World.instance.Seed + 206) / 64f
                                    )
                                );
                        ...
                        float VegetationMask = Mathf.PerlinNoise(
                            (ox + position.x + World.instance.Seed + 500) / 128f,
                            (oz + position.z + World.instance.Seed + 900) / 128f
                        );
                        ...
                        if (perlinMask2D > perlin2DValue && perlin3D > 0.5 && perlin2DValue > 52)
                        {
                            ...
                            if (
                                oy + position.y > perlinMask2D - 5 + r / 2
                                && y + position.y < perlinMask2D
                            )
                            {
                        ...
                        {
                            if (
                                perlin2DValue - 6 * offset + r >= (oy + position.y)
                                && blocks[index] == Block.Air
                            )
                                blocks[index] = Block.Stone;

                            if (
                                perlin2DValue > (oy + position.y)
                                && perlin2DValue - 6 + r < (oy + position.y)
                                && blocks[index] == Block.Air
                            )
                                blocks[index] = Block.Dirt;
                        }
                        ...
                        if (
                            perlin2DValue == (oy + position.y) /* && oy + position.y >= waterLevel && blocks[index] == Block.Dirt */
                        )
                            blocks[index] = Block.Grass;
                        ...
                        if (
                            (oy + position.y)
                                > perlin2DValue
                                    - (
                                        Mathf.Clamp(waterLevel - perlin2DValue + offset * 4, 0, 4)
                                        - Mathf.Abs(r)
                                    )
                            && (oy + position.y) <= perlin2DValue
                        )
                            blocks[index] = Block.Sand;

                        if (
                            perlin2DValue <= waterLevel
                            && waterLevel > (oy + position.y)
                            && (oy + position.y) > perlin2DValue - (waterLevel - perlin2DValue)
                            && blocks[index] == Block.Air
                        )
    ...
    public void UpdateChunk()
    {
        ...
        Vector3 viewer = new Vector3(
            World.viewerPosition.x,
            World.viewerPosition.y,
            World.viewerPosition.z
        );
    ...
    public Block GetBlockAt(float x, float y, float z)
    {
        ...
        if (IsPointInChunk(x, y, z))
            return blocks[
                (int)(
                    (x / offset) * Chunk.BlockPerChunk * Chunk.BlockPerChunk
                    + (y / offset) * Chunk.BlockPerChunk
                    + (z / offset)
                )
            ];
        return Block.Air;
    ...
    public bool SetBlockAt(Vector3 point, Vector3 normal, Block block, bool setBlockMode)
    {
        if (setBlockMode == false)
        {
            ...
            {
                blocks[
                    x * Chunk.BlockPerChunk * Chunk.BlockPerChunk + y * Chunk.BlockPerChunk + z
                ] = Block.Air;
                Chunk chunk = this;
